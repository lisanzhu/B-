...
class MovieHandler(xml.sax.ContentHandler):
    ...
    def __init__(self):
        ...
        self.n= 0    # 文本格式化之后，指定第几篇文章
        self.num=[]  # 存放和文章标签同级的标签所在的行数
        self.attr={}
        self.size=0

        # 存放首字母

        self.bPTree_article=Bptree(100,50)
        # 存放文章名的列表

        self.bPTree_author = Bptree(100, 50)  # 以作者名为索引的B+树 值为一个列表(每篇文章名)
        self.author_dic = {}  # 建立作者名为索引时将其所有的放入到一个字典之中，直到最后读取至dblp时将其插入到B+树中
        self.title_list = []
        self.CurrentData = ""
        # 与article同级的标签，代表文章的开始
        self.article_levelong_tag = ["article", "book", "proceedings", "inproceedings", "www", "mastersthesis",
        ...
        self.ignore = {'For', 'for', 'And', 'and', 'On', 'on', 'the', 'The', 'a', 'A', 'Of', 'of', 'With', 'with', 'An',
                       'an', 'In', 'in', 'At', 'at', 'Have', 'have', 'Is', 'is', 'Are', 'are', 'Has', 'has', 'Not',
                       'not', 'By', 'by', 'Into', 'into', 'Through', 'through', 'Off', 'off', 'Out', 'out','from','to','Using', 'using','based' ,'systems' ,'System','Data' ,'Analysis' ,'Systems','Based',
                       'networks','Networks','system',}  # 忽视掉的热点词
    ...
    def endElement(self, tag):
        if tag in self.article_level_tag:  # 如果标签在和article同一个标签之下
            ...
            if self.dic:  # 如果字典不为空
                ...
                # self.num.append(self.n)  # 文章末对应行数

                if 'title' in self.dic:

                    if not self.bPTree_article.search(self.dic['title'][0], self.dic['title'][0]):
                        # self.bPTree_article.insert(KeyValue(self.dic['title'][0], [self.num.copy()]))
                        self.bPTree_article.insert(KeyValue(self.dic['title'][0], [self.n]))
                    else:
                        self.bPTree_article.search(self.dic['title'][0], self.dic['title'][0]).append(
                            self.n)


                    self.title_list.append(self.dic['title'][0])

                    if 'year' in self.dic:
                        trantab = str.maketrans({key: None for key in string.punctuation})  # 转换功能将所有的标点转化为none
                        t = self.dic['title'][0].translate(trantab)

                        for word in t.split():
                            if len(word)>=5:
                                if self.dic['year'][0] in self.hot_word:
                                    if word not in self.ignore:  # 排除介词
                                        if word in self.hot_word[self.dic['year'][0]]:  # 如果单词已经存在
                                            self.hot_word[self.dic['year'][0]][word] = self.hot_word[self.dic['year'][0]][
                                                                                           word] + 1
                                        else:
                                            self.hot_word[self.dic['year'][0]][word] = 1
                                else:
                                    if word not in self.ignore:  # 排除介词
                                        self.hot_word[self.dic['year'][0]] = {}
                                        self.hot_word[self.dic['year'][0]][word] = 1

                if 'author' in self.dic:
                    # 如果存在作者则首先加入到一个大字典中
                    # 首先遍历文章所有作者
                    for i in range(len(self.dic['author'])):
                        # 若不存在作者，即发现一个作者到目前位置只有这一篇文章，进行构建
                        if self.dic['author'][i] not in self.author_dic:
                            # self.author_dic[self.dic['author'][i]] = [self.dic['title']]
                            self.author_dic[self.dic['author'][i]]=[self.n]
                        # 若存在，则将文章名加入到作者写的文章的列表中
                        else:
                            self.author_dic[self.dic['author'][i]].append(self.n)

        if tag == 'dblp':  # 最后若读到文件结尾则将大字典中所有的作者及其文章加入到以作者为索引的B+树之中，逐个插入
            for i in self.author_dic.keys():
                # 向大的作者树中插入所有的键值
                self.bPTree_author.insert(KeyValue(i, self.author_dic[i].copy()))

        elif tag == "author":  # 把拼接完的作者名记录到字典中
            if self.name_string not in self.author:   # 记录作者写的文章数，前一百功能。
...
def test():
    ...
    f = open('data_articleTree.txt', 'wb')
    print(2)
    pickle.dump(Handler.bPTree_article, f)
    print(3)
    f.close()

    f = open('data_authorTree.txt', 'wb')
    pickle.dump(Handler.bPTree_author, f)
    f.close()

    f = open('data_author.txt', 'wb')
    pickle.dump(Handler.author, f)
    f.close()

    f = open('title_list.txt', 'wb')
    pickle.dump(Handler.title_list, f)
    f.close()

    f = open('hot_word.txt', 'wb')
    pickle.dump(Handler.hot_word, f)
    f.close()
